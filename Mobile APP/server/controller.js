...
module.exports = async (app, myDb) => {
    app.post('/signup', async (req, res) => {
        ...
        collection.findOne(query, (err, result) => {
            if (result == null) {
                ...
                res.cookie('cookieName', newUser.Phone + "_" + randomNumber, { maxAge: 900000, httpOnly: true });
    ...
    app.post("/phone", async (req, res) => {
        try {
            const result = await myDb.collection("signups").findOne({ Phone: req.body.phone })
            if (result != null) {
                const objToSend = {
                    name: result.Name,
                    email: result.Email,
    ...
    app.post("/getpersonal", async (req, res) => {
        try {
            const result = await myDb.collection("personals").findOne({ Phone: req.body.Phone })
            if (result == null) {
                return res.status(404).send();
            }
            const objToSend = {
    ...
    app.post("/getEducation", async (req, res) => {
        try {
            const result = await myDb.collection("academics").findOne({ Phone: req.body.Phone })
            if (result == null) {
                return res.status(404).send();
            }
            const objToSend = {
    ...
    app.post("/education", async (req, res) => {
        ...
        try {
            ...
            await myDb.collection("academics").updateOne({ Phone: req.body.Phone }, User)
            return res.status(200).send();
        } catch (err) {
            res.status(500).send();
            console.log(err);
    ...
    app.post("/resetpassword", async (req, res) => {
        try {
            ...
            await myDb.collection("signups").updateOne({ Phone: req.body.phone }, User)
            return res.status(200).send();
