...
const Documents = ({match}) => {
  ...
  async function fetchData() {
    if(isBusy)
    {
      ...
      setData(res.data)
      setModalState(Object.assign({}, ...res.data.map(k => ({ [k]: false }))))
      setBusy(false)
    }
  }
          ...
          <CDataTable
            ...
            scopedSlots = {{
              'Preview':
                (item)=>(
                  <td>
                    <CImg src={uri+`/document/${item}/user/${match.params.id}`} thumbnail />
                  </td>
                ),
                'Document Name':
                (item)=>(
                ...
                (item)=>(
                    <td>
                        <CButton color="primary" onClick={()=>{toggleModal(item)}} className="m-2">
                            View {item}
                        </CButton>
                        <CModal show={modalState[item]} onClose={()=>{toggleModal(item)}}>
                            <CModalHeader closeButton>{item}</CModalHeader>
                            <CModalBody>
                                <CImg src={uri+`/document/${item}/user/${match.params.id}`} fluidGrow />
                            </CModalBody>
                            <CModalFooter>
